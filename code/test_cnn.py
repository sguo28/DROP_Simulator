...
if __name__ == '__main__':
    ...
    for s in range(5):
        model_file='logs/test/cnn_dqn_model/dqn_fh_0.005_{}_20160_{}.pkl'.format(OPTION_DIM,11200+s)
        trial,step,OPTION_DIM,LEARNING_RATE=dqn_agent.load_parameter(model_file)
        # if step<20000:
        #     print(model_file,'invalid model loaded')
        #     continue
        #
        # if trial<8002 or trial>8002:
        #     continue

        print('now using network:',model_file)
        if 1:  # 10 runs
            trial = trial
            print(
                            ...
                            if len(local_state_batches) > 0:
                                ...
                                if OPTION_DIM > 0:
                                    terminal_flag, v_idx, opt_idx = dqn_agent.is_terminal(local_state_batches,
                                                                                          tick)  # return terminal states and not choosing options
