...
from config.setting import HEX_SHP_PATH, CS_SHP_PATH, NUM_NEAREST_CS, TRIP_FILE, TRAVEL_TIME_FILE, \
    TIMESTEP, START_OFFSET, SIM_DAYS,  START_TIME, TRAINING_CYCLE, STORE_TRANSITION_CYCLE, LOW_SOC_THRESHOLD,\
    HIGH_SOC_THRESHOLD
from dqn_option_agent.h_agent import H_Agent
from dqn_option_agent.dqn_option_agent_dry_run import DeepQNetworkOptionAgent
from dqn_agent.dqn_agent_with_cnn.cnn_dqn_agent import DeepQNetworkAgent
from simulator.simulator_dry_run import Simulator

# def dummy_get_actions(states, num_valid_relos):
#     relocation_dim = 7
#     premitive_action_dim = 12
#     full_action_values = np.random.random((len(states), premitive_action_dim))
#     for i, state in enumerate(states):
#         full_action_values[i][num_valid_relos[i]:relocation_dim] = -1
#         if state[-1] > HIGH_SOC_THRESHOLD:
#             full_action_values[i][relocation_dim:] = -1  # no charging, must relocate
#         elif state[-1] < LOW_SOC_THRESHOLD:
#             full_action_values[i][:relocation_dim] = -1  # no relocation, must charge
#     return np.argmax(full_action_values, 1).tolist()
...
if __name__ == '__main__':
    ...
    if SIM_DAYS > 0:
        ...
        dqn_agent = DeepQNetworkOptionAgent(simulator.hex_diffusions,NUM_OPTION,args.isoption, args.islocal, args.ischarging) if args.isoption else \
        ...
        with open('logs/parsed_results_%s_%s_%s.csv' % (isoption, islocal, ischarging), 'w') as f, open(
            ...
            for episode in range(10): # we need at least 2e4 steps = ~ 14 days.
